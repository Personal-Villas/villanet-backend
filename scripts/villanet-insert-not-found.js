 ...
 *
 * IMPORTANTE: Solo se insertan propiedades que en Guesty figuren como
 * isListed = true AND isActive = true. Si una propiedad está desactivada
 * en Guesty, se reporta en "Revisión Manual" y NO se inserta, para evitar
 * que el URL Guardian la marque como falla.
 *
 * Para cada listing válido encontrado:
 ...
 *   node scripts/villanet-insert-not-found.js --list scripts/docs/not-found-insert-list.json
 *
 * Archivos de referencia:
 *   Input : scripts/docs/not-found-insert-list.json
 *   Output: scripts/docs/villanet-insert-not-found-report.json
 */
...
const { values: args } = parseArgs({
  options: {
    list:      { type: 'string',  default: 'scripts/docs/not-found-insert-list.json' },
...
Usage: node scripts/villanet-insert-not-found.js [options]
  ...
  --list      Path al JSON con la lista de IDs/nombres  (default: scripts/docs/not-found-insert-list.json)
...
function extractListingFields(listing) {
  ...
  if (!heroImage && listing?.pictures?.[0])
    heroImage = listing.pictures[0].large || listing.pictures[0].original || null;
...
async function main() {
  ...
  const reportPath = path.resolve('scripts/docs/villanet-insert-not-found-report.json');
  ...
  for (const item of known_ids) {
    ...
    // Verificar que esté activa y listada en Guesty antes de insertar.
    // Ambos defaultean a false para no activar propiedades no disponibles.
    const isListed = listing.isListed ?? false;
    const isActive = listing.isActive ?? false;

    if (!isListed || !isActive) {
      console.log(`⚠️  inactiva en Guesty (isListed=${isListed}, isActive=${isActive}) — omitida`);
      results.needs_manual.push({
        ...item,
        reason: `Inactiva en Guesty (isListed=${isListed}, isActive=${isActive})`,
      });
      continue;
    }
  ...
  for (const item of search_by_name) {
    ...
    // Filtrar solo activas y listadas. Ambos defaultean a false para no
    // activar propiedades inactivas que generarían fallas en el URL Guardian.
    const active = candidates.filter(l => (l.isListed ?? false) && (l.isActive ?? false));

    if (!active.length) {
      const totalCandidates = candidates.length;
      const reason = totalCandidates > 0
        ? `${totalCandidates} resultado(s) en Guesty pero ninguno activo/listado`
        : 'no encontrado en búsqueda Guesty';
      console.log(`⚠️   ${reason} — requiere revisión manual`);
      results.needs_manual.push({ ...item, reason });
      continue;
    }

    if (active.length > 1) {
      const names = active.map(l => `"${l.nickname || l.name}" (${l._id})`).join(', ');
      console.log(`⚠️   ${active.length} candidatos activos → revisión manual: ${names}`);
      results.needs_manual.push({
        ...item,
        candidates: active.map(l => ({ id: l._id, name: l.nickname || l.name })),
      });
      continue;
    }
  ...
  if (results.needs_manual.length) {
    ...
    for (const r of results.needs_manual) {
      if (r.candidates) {
        console.log(`    - "${r.name}" [${r.destination}]: múltiples candidatos activos:`);
